...
export default function VerifyPage() {
  ...
  const handleZKPScan = useCallback(async (raw: string) => {
    ...
    // Old zkp: format — redirect user to correct flow
  ...
  const handleStandardScan = useCallback(async (raw: string) => {
    ...
    try {
      ...
      if (verifyMode === "quick") {
        ...
        setSessionCount(c => c + 1);
        setState({ status: "approved", holderName: t.holderName, category: t.category, seat: t.seat, txHash });
      } else {
        // CNIC mode — show details, wait for CNIC input
        setState({
          status: "match_found",
          holderName: t.holderName, category: t.category, seat: t.seat,
          matchName: matchName as string, ticketId, matchAddr, cnicHash: t.cnicHash,
        });
      }
    } catch (e) {
  ...
  function handleCnicChange(raw: string) {
    ...
    const hash = keccak256(toBytes(digits));
    setState({ ...state, status: hash === state.cnicHash ? "hash_match" : "hash_mismatch" });
  }
  ...
  if (!isConnected) {
    return (
      <div className="max-w-lg mx-auto px-4 py-20 flex flex-col items-center gap-6">
        ...
        <h1 className="font-display font-bold text-xl tracking-widest text-white">VENUE VERIFICATION</h1>
        <p className="font-body text-sm text-muted text-center">Connect your wallet to verify tickets.</p>
        <ConnectButton />
      </div>
    );
  }
    ...
    <div className="max-w-2xl mx-auto px-4 sm:px-6 py-10">
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="font-body text-sm text-muted ml-4">Scan fan QR codes to verify entry</p>
        </div>
        {sessionCount > 0 && (
      ...
      {selectedMatch ? (
        <div className="card-glass rounded overflow-hidden mb-4">
          ...
          <div className="p-6">
            ...
            {/* ZKP mode hint (shown below code entry) */}
              ...
              <div className="mb-4 p-3 rounded border flex items-start gap-2"
                ...
                <div>
                  ...
                  <p className="font-mono text-[10px] text-muted mt-1">
                    ...
                    The proof is verified on-chain via Groth16 verifier contract.
                  </p>
                </div>
              </div>
            )}
            ...
            {state.status === "approved" && (
              ...
              >
                ...
                <InfoRow label="SEAT" value={state.seat} />
                {!state.isZkp && (
                  <InfoRow label="CATEGORY" value={typeof state.category === "number" ? categoryLabel(state.category as number) : state.category} />
                )}
                {state.isZkp && (
                ...
                {state.txHash && (
                  <a href={`${process.env.NEXT_PUBLIC_BLOCK_EXPLORER}/tx/${state.txHash}`}
            ...
            {(state.status === "match_found" || state.status === "hash_match" || state.status === "hash_mismatch") && (
              <div>
                ...
                <div className="mb-3">
                  <label className="block font-mono text-xs text-muted mb-1.5 tracking-wider uppercase">Enter Fan CNIC</label>
                  <input className="input-field" placeholder="12345-6789012-3" value={cnic}
                    inputMode="numeric" onChange={e => handleCnicChange(e.target.value)} autoFocus />
                </div>
                ...
                <div className="flex gap-3">
                  {state.status === "hash_match" && (
                    <button onClick={handleApprove} className="btn-primary flex-1 justify-center">✓ APPROVE ENTRY</button>
                  )}
